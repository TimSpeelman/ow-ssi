...
import { OWVerifier } from "./OWVerifier";
 ...
 */
export class OWVerifyAgent {

    constructor(private owVerifier: OWVerifier, private api: OWAPI) { }

    async sendVerifyRequest(subject_mid: string, req: OWVerifyRequest): Promise<OWVerifyResponse> {
        const message = JSON.stringify(req);
        const success = await this.api.sendMessage(subject_mid, message);
        if (!success) {
            throw new Error(`Peer ${subject_mid} unknown`);
        }

    }

    verify(req: OWVerifyRequest, res: OWVerifyResponse): Promise<boolean> {
